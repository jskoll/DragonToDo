...
7. `src/components/ThemeSelector.tsx` - dead code from deleted App.tsx

SCSS files:
8. `src/components/TodoList.scss`
9. `src/components/TodoItem.scss`
10. `src/components/TodoForm.scss`
11. `src/components/FilterBar.scss`
12. `src/components/StatsPanel.scss`
13. `src/components/EditTodoModal.scss`
14. `src/components/ThemeSelector.scss` - dead code from deleted App.tsx

Test files for plain components:
15. `src/components/__tests__/TodoItem.test.tsx`
16. `src/components/__tests__/TodoForm.test.tsx`
17. `src/components/__tests__/ThemeSelector.test.tsx`
...
## Item 2: Delete Unused CSS Theme System
...
9. `src/types/theme.ts` - Theme type definitions (ThemeName, Theme interface)

Also delete any remaining SCSS files that are only used by the dead code:
10. `src/styles/_index.scss`
11. `src/styles/_variables.scss`
12. `src/styles/_mixins.scss`
...
## Item 3: Fix IPC Event Listener Memory Leaks
...
## Item 4: Fix Stale Closure Bug in updateTodo
...
## Item 5: Decompose App_Complete.tsx with Custom Hooks
...
## Item 6: Standardize UUID Generation and Remove uuid Dependency
...
## Item 7: Move @types and electron to devDependencies
...
## Item 8: Eliminate any Type Usages
...
## Item 9: Remove Duplicate ElectronAPI Interface
...
## Item 10: Guard electron-reloader for Development Only
...
## Item 11: Increase PBKDF2 Iterations for Encryption Security
...
## Item 12: Use Magic Bytes for Encrypted File Detection
...
## Item 13: Extract Reminder Validation into a Custom Hook
...
## Item 14: Complete the setupTests.ts Mock for Electron API
...
## Item 15: Fix Inconsistent React.useEffect Usage
...
## Item 16: Define IPC Channel Names as Constants
...
## Item 17: Add ESLint and Prettier Configuration
...
## Item 18: Add .editorconfig for Consistent Editor Settings
...
## Item 19: Add Node.js Engine Requirements to package.json
...
## Item 20: Fix File Extension Forcing Behavior
...
## Item 21: Decompose main.ts into Focused Modules
...
## Item 22: Update Test Files to Remove Dead Code Test References
...
**Phase 6 - Security (Items 11-12, 20)**
Improve encryption iterations, file format detection, and extension behavior.

**Phase 7 - Cleanup (Item 22)**
Final test cleanup after all other changes are complete.

---

*Each prompt above is self-contained and does not depend on any other item being completed first. However, if items 1-2 (dead code removal) are done before items 8, 13, or 15, those later items will be simpler since they won't need to consider deleted files.*
